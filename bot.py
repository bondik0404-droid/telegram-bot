...
def ask_ai(prompt: str, system: str = CHANNEL_SYSTEM) -> str:
...
def generate_post(topic: str) -> str:
    return ask_ai(f"Напиши пост для Telegram-канала о бизнесе/маркетинге. "
                  f"Структура: заголовок с эмодзи → 3-5 тезисов → призыв к действию. До 900 символов.\n\nТема: {topic}")

def moderate(text: str) -> bool:
    r = ask_ai(f"Сообщение в деловом канале. Спам/мат/реклама — ответь УДАЛИТЬ, иначе — ОК.\n\n{text}")
...
Доп. информация: {data.get('extra')}"""
    return ask_ai(prompt, system=DD_SYSTEM)
...
async def dd_extra(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ Анализирую данные, формирую отчёт... (~10 сек)")
    try:
        ...
        logger.error(e)
        await update.message.reply_text("❌ Ошибка при генерации отчёта. Попробуйте ещё раз: /dd")
    ctx.user_data["dd"] = {}
...
async def handle_comment(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.text.strip().endswith("?"):
        await msg.reply_text(ask_ai(msg.text))
